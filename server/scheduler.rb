...
thread = Thread.new do
  ...
  if defined? FORTUM_DEV4_REGRESSION_TESTS_JOB
    begin
      scheduler.cron FORTUM_DEV4_REGRESSION_TESTS_JOB[:cron] do
        FORTUM_DEV4_REGRESSION_TESTS_JOB[:suites_environments].each do |job|
          test_execution = TestExecution.new
          test_execution.test_suite_name = job[:suite]
          test_execution.environment_name = job[:environment]
          test_execution.for = 'SCHEDULER'
